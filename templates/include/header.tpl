...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script type="text/javascript" src="{$url_base}ui/elements/jquery-1.7.2.min.js"></script>      
    ...
    <body>
        <table id="contentcontainer" cellpadding="0" cellspacing="0">
            ...
            <tr id="maintr2">
                ...
                <td id="content">
                    <ul class="breadcrumb">
                        {if $admin && $smarty.request.page==='event'}
                            {if  $visibility=='private'}
                                {assign var=private value="private_visibility"}
                                <div class="visibility_bar_private">
                                    <div class="event_visibility_change">
                                        <a class="regular" href="{url page=event action=publish view=$smarty.get.view id=$smarty.get.id cmd=publish}">{translate id=publish}<img src="ui/elements/publish.png" alt="Publish" border="0" /></a>  
                                    </div>
                                    {translate id=visibility_private}
                                </div>
                            {else} 
                                <div class="visibility_bar_public">
                                    <div class="event_visibility_change">
                                        <a class="regular" href="{url page=event action=publish view=$smarty.get.view id=$smarty.get.id cmd=unpublish }">{translate id=unpublish}<img src="ui/elements/unpublish.png" alt="Unpublish" border="0" /></a>  
                                    </div>
                                    {translate id=visibility_public}
                                </div>
                            {/if}   
