...
function processForm() {
    ...
    foreach ($classes as $class) {
        ...
        //$firephp->info($last, 'last');
        $limit = @$_POST['player_limit' . $last];
        //if ((int) $limit <= 0)
        //    $problems['player_limit_'] = translate('FormError_NotPositiveInteger');
    ...
    $result = EditEvent($eventid, $name, $venue, $duration, $contact, $tournament, $level, $start, $signup_start, $signup_end, $state, $requireFees, $player_limit, $class_limits);
    ...
    $result = SetClasses($eventid, $classes, $class_limits);
    if (is_a($result, 'Error'))
        return $result;
