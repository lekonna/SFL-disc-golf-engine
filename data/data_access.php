...
function GetUsers($searchQuery = '', $sortOrder = '') {
    ...
    if (mysql_num_rows($result) > 0) {
        while ($row = mysql_fetch_assoc($result)) {
            $temp = new User($row['id'], htmlspecialchars($row['Username'], ENT_NOQUOTES, 'UTF-8'), $row['Role'],
                            data_GetOne($row['UserFirstname'], $row['pFN']),
                            data_GetOne($row['UserLastname'], $row['pLN']),
                            data_GetOne($row['UserEmail'], $row['pEM']), $row['Player']
            );
...
function GetPlayerUsers($query = '', $sortOrder = '') {
    ...
    if (mysql_num_rows($result) > 0) {
        while ($row = mysql_fetch_assoc($result)) {
            $temp = new User($row['id'], htmlspecialchars($row['Username'], ENT_NOQUOTES, 'UTF-8'), $row['Role'], htmlspecialchars($row['UserFirstname'], ENT_NOQUOTES, 'UTF-8'), htmlspecialchars($row['UserLastname'], ENT_NOQUOTES, 'UTF-8'), $row['UserEmail'], $row['Player']);
            $retValue[] = $temp;
        }
    }
    mysql_free_result($result);
...
function GetUserPlayer($userid) {
    ...
    $query = data_query("SELECT :Player.player_id id, pdga PDGANumber, sex Sex, YEAR(birthdate) YearOfBirth, firstname, lastname, email, club_id
    ...
    if (mysql_num_rows($result) == 1) {
        $row = mysql_fetch_assoc($result);
        $retValue = new Player($row['id'], $row['PDGANumber'], $row['Sex'], $row['YearOfBirth'], $row['firstname'], $row['lastname'], $row['email'], $row['club_id']);
    }
    ...
    if ($user) {
        ...
        $query = data_query("SELECT :Event.id, :Venue.Name AS Venue, :Venue.id AS VenueID, Tournament, Level, :Event.Name, UNIX_TIMESTAMP(Date) Date, Duration,
                               ...
                               :Level.Name LevelName,
                                    :EventManagement.Role AS Management, :Participation.Approved, :Participation.EventFeePaid, :Participation.Standing, :Waitinglist.Signup_number as Waiting_number, :Waitinglist.Classification as Waiting_class
                                         FROM :Event                                         
                                         ...
                                         LEFT JOIN :Waitinglist ON :Waitinglist.Event = :Event.id AND :Waitinglist.Player = $playerid
 ...
 */
function CreateEvent($name, $venue, $duration, $contact, $tournament, $level, $start, $signup_start, $signup_end, $classes, $td, $officials, $requireFees, $player_limit, $class_limits) {
    $retValue = null;
    ...
    $query = datA_query("INSERT INTO :Event (Venue, Tournament, Level, Name, Date, Duration, SignupStart, SignupEnd, ContactInfo, FeesRequired, PlayerLimit) VALUES
                     (%d, %s, %d, '%s', FROM_UNIXTIME(%d), %d, FROM_UNIXTIME(%s), FROM_UNIXTIME(%s), '%s', %d, %d)", esc_or_null($venue, 'int'), esc_or_null($tournament, 'int'), esc_or_null($level, 'int'), mysql_real_escape_string($name), (int) $start, (int) $duration, esc_or_null($signup_start, 'int'), esc_or_null($signup_end, 'int'), mysql_escape_string($contact), $requireFees, $player_limit);
    ...
    if (mysql_query($query)) {
        ...
        $retValue = SetClasses($eventid, $classes, $class_limits);
...
function EditUserInfo($userid, $email, $firstname, $lastname, $gender, $pdga, $dobyear, $club = 0) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    ...
    $user_query = data_query("UPDATE :User SET UserEmail = %s, UserFirstName = %s, UserLastName = %s WHERE id = %d", esc_or_null($email), esc_or_null(data_fixNameCase($firstname)), esc_or_null(data_fixNameCase($lastname)), (int) $userid);
    ...
    if ($player) {
        ...
        $plr_query = data_query("UPDATE :Player SET sex = %s, pdga = %s,
                                 ...
                                 email = %s, club_id = %d
                                 WHERE player_id = %d", strtoupper($gender) == 'M' ? "'male'" : "'female'", esc_or_null($pdga, 'int'), (int) $dobyear . '-1-1', esc_or_null(data_fixNameCase($firstname)), esc_or_null(data_fixNameCase($lastname)), esc_or_null($email), $club, (int) $playerid);
...
function GetEventClasses($event) {
    ...
    if (mysql_num_rows($result) > 0) {
        while ($row = mysql_fetch_assoc($result)) {
            ...
            $class = new Classification($row);
            if ($class->class_player_limit != 0) {
                if (checkIfPlayersInWaitingList($event, $class->id)) {
                    $openPos = 0;
                } else {
...
function EditEvent($eventid, $name, $venuename, $duration, $contact, $tournament, $level, $start, $signup_start, $signup_end, $state, $requireFees, $player_limit, $class_limits) {
    ...
    $query = data_query("UPDATE `:Event` SET `Venue` = %d, `Tournament` = %s, Level = %d, `Name` = '%s', `Date` = FROM_UNIXTIME(%d),
                       ...
                       ActivationDate = FROM_UNIXTIME( %s), ResultsLocked = FROM_UNIXTIME(%s), ContactInfo = '%s', FeesRequired = %d, PlayerLimit = %d
                       
                       WHERE id = %d", $venueid, esc_or_null($tournament, 'int'), $level, mysql_real_escape_string($name), (int) $start, (int) $duration, esc_or_null($signup_start, 'int'), esc_or_null($signup_end, 'int'), $activation, $locking, mysql_real_escape_string($contact), $requireFees, (int) $player_limit, (int) $eventid);
...
function SetTD($eventid, $td) {
    ...
    if (isset($eventid) and isset($td)) {
        ...
        $query = data_query("INSERT INTO :EventManagement (User, Event, Role) VALUES (%d, %d, '%s');", (int) $td, (int) $eventid, 'td');
...
function SetOfficials($eventid, $officials) {
    ...
    if (isset($eventid)) {
        ...
        foreach ($officials as $official) {
            $query = data_query("INSERT INTO :EventManagement (User, Event, Role) VALUES (%d, %d, '%s');", (int) $official, (int) $eventid, 'official');
            if (!mysql_query($query)) {
                $err = new Error();
                $err->title = "error_db_query";
                $err->description = translate("error_db_query_description");
 ...
 */
function SetClasses($eventid, $classes, $class_limits) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    ...
    if (isset($eventid)) {
        ...
        foreach ($classes as $class) {
            //arttu 14.3.2012
            $class_player_limit = 0;
            foreach ($class_limits as $key => $value) {
                if ($key == $class) {
                    $class_player_limit = $value;
                    break;
                }
            }
            $query = data_query("INSERT INTO :ClassInEvent (Classification, Event, ClassPlayerLimit) VALUES (%d, %d, %d);", (int) $class, (int) $eventid, (int) $class_player_limit);
            if (!mysql_query($query)) {
                return Error::Query($query);
            }
        }
...
function SetRounds($eventid, $rounds, $deleteRounds = array()) {
    ...
    foreach ($rounds as $round) {
        ...
        if (empty($roundid) || $roundid == '*') {
            // Create new round
            $query = data_query("INSERT INTO :Round (Event, Course, StartType, StartTime, `Interval`, ValidResults) VALUES (%d, %s, '%s', FROM_UNIXTIME(%d), %d, %d);", $r_event, esc_or_null($r_course, 'int'), $r_starttype, $r_starttime, $r_interval, $r_validresults);
...
function GetOrSetRoundCourse($roundid) {
    ...
    $query = data_query("SELECT Course FROM :Round WHERE id = %d", (int) $roundid);
    $result = mysql_query($query);
    if (mysql_num_rows($result) == 1) {
        $row = mysql_fetch_assoc($result);
        $course = $row['Course'];
    ...
    if (!isset($courseid)) {
        ...
        $query = data_query("UPDATE :Round SET Course = %d WHERE id = %d;", $courseid, $roundid);
        if (!mysql_query($query)) {
            $err = new Error();
            $err->title = "error_db_query";
            $err->description = translate("error_db_query_description");
...
function SetCourseHoles($courseid, $holes) {
    ...
    for ($holenumber = 1; $holenumber <= $holes; $holenumber++) {
        ...
        $query = data_query("SELECT id, Par, Length FROM :Hole WHERE Course = %d AND HoleNumber = %d", (int) $courseid, $holenumber);
        $result = mysql_query($query);
        ...
        if ($nbr_of_results == 1) {
            ...
            $query = data_query("INSERT INTO :Hole (Course, HoleNumber, Par, Length) VALUES (%d, %d, %d, %d);", $courseid, $holenumber, $h_par, $h_length);
            if (!mysql_query($query)) {
                $err = new Error();
                $err->title = "error_db_query";
                $err->description = translate("error_db_query_description");
...
function SetPlayerParticipation($playerid, $eventid, $classid) {
    ...
    $query = data_query("INSERT INTO :Participation (Player, Event, Classification) VALUES (%d, %d, %d);", (int) $playerid, (int) $eventid, (int) $classid);
    if (!mysql_query($query)) {
        $err = new Error();
        $err->title = "error_db_query";
        $err->description = translate("error_db_query_description");
...
function CancelSignup($eventid, $playerid) {
    ...
    $query = data_query("DELETE FROM :Participation  WHERE Player = %d AND Event = %d", (int) $playerid, (int) $eventid);
    if (!mysql_query($query)) {
        return Error::Query($query);
    }
...
function GetVenueId($venue) {
    ...
    $query = data_query("SELECT id FROM :Venue WHERE Name = '%s'", mysql_real_escape_string($venue));
    $result = mysql_query($query);
    if (mysql_num_rows($result) >= 1) {
        $row = mysql_fetch_assoc($result);
        $venueid = $row['id'];
    ...
    if (!isset($venueid)) {
        // Create a new venue
        $query = data_query("INSERT INTO :Venue (Name) VALUES ('%s');", mysql_real_escape_string($venue));
        if (mysql_query($query)) {
...
function CreateNewsItem($eventid, $title, $text) {
    ...
    $query = data_query("INSERT INTO :TextContent(Event, Title, Date, Content, Type) VALUES(%d, '%s', NOW(), '%s', 'news')", (int) $eventid, mysql_real_escape_string($title), mysql_real_escape_string($text));
...
function EditNewsItem($itemid, $title, $text) {
    ...
    $query = data_query("UPDATE :TextContent SET Title = '%s', Content = '%s' WHERE id = %d", mysql_real_escape_string($title), mysql_real_escape_string($text), (int) $itemid);
...
function SetUserDetails($user) {
    ...
    if (is_a($user, "User")) {
        ...
        if (!GetUserId($user->username)) {
            ...
            $query = data_query("INSERT INTO :User (Username, UserEmail, Password, Role, UserFirstName, UserLastName, Player) VALUES (%s, '%s', '%s', '%s', '%s', '%s', %s);", $u_username_quoted, $u_email, $u_password, $u_role, $u_firstname, $u_lastname, esc_or_null($user->player, 'int'));
...
function SetPlayerDetails($player) {
    ...
    if (is_a($player, "Player")) {
        ...
        $query = data_query("INSERT INTO :Player (pdga, sex, lastname, firstname, birthdate, email, club_id) VALUES (
                            %s, '%s', %s, %s, '%s', %s, %d
                            );", esc_or_null($player->pdga), $player->gender == 'M' ? 'male' : 'female', esc_or_null(data_fixNameCase($player->lastname)), esc_or_null(data_fixNameCase($player->firstname)), (int) $player->birthyear . '-1-1', esc_or_null($player->email), $player->club_id
        );
...
function EditLevel($id, $name, $method, $available) {
    ...
    $query = data_query("UPDATE :Level SET Name = '%s', ScoreCalculationMethod = '%s', Available = %d WHERE id = %d", mysql_real_escape_string($name), mysql_real_escape_string($method), $available ? 1 : 0, (int) $id);
...
function CreateLevel($name, $method, $available) {
    ...
    $query = data_query("INSERT INTO :Level (Name, ScoreCalculationmethod, Available) VALUES ('%s', '%s', %d);", mysql_real_escape_string($name), mysql_real_escape_string($method), $available ? 1 : 0);
...
function EditTournament($id, $name, $method, $level, $available, $year, $description) {
    ...
    $query = data_query("UPDATE :Tournament SET Name = '%s', ScoreCalculationMethod = '%s', Level = %d, Available = %d, Year = %d,
                       Description = '%s'
                       WHERE id = %d", mysql_real_escape_string($name), mysql_real_escape_string($method), (int) $level, $available ? 1 : 0, (int) $year, mysql_real_escape_string($description), (int) $id);
...
function CreateTournament($name, $method, $level, $available, $year, $description) {
    ...
    $query = data_query("INSERT INTO :Tournament(Name, ScoreCalculationMethod, Level, Available, Year, Description) VALUES('%s', '%s', %d, %d, %d, '%s')", mysql_real_escape_string($name), mysql_real_escape_string($method), (int) $level, $available ? 1 : 0, (int) $year, mysql_real_escape_string($description));
...
function GetEventQueuers($eventId, $sortedBy, $search) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    ...
    $eventId = (int) $eventId;
    /*
      $classes = GetEventClasses($eventId);
    ...
    $sortOrder = data_CreateSortOrder($sortedBy, array('name' => array('LastName', 'FirstName'), 'class' => 'ClassName', 'LastName' => true, 'FirstName' => true, 'birthyear' => 'YEAR(birthdate)', 'pdga', 'gender' => 'sex', 'Username', 'signupNumber' => 'Signup_number', 'signupDate' => 'UNIX_TIMESTAMP(Signup_Timestamp)'));
    ...
    $query = "SELECT :User.id AS UserId, Username, Role, UserFirstName, UserLastName, UserEmail, :Player.firstname pFN, :Player.lastname pLN,
                               ...
                               :Waitinglist.id AS WaitinglistID, :Waitinglist.Signup_number,
                               UNIX_TIMESTAMP(Signup_Timestamp) Timestamp, :Classification.id AS ClassId, sfl_clubs.club_short AS Club_short
                  FROM :User
                  INNER JOIN :Player ON :Player.player_id = :User.Player
    ...
    $query = data_query($query, data_ProduceSearchConditions($search, array('Classification', 'LastName', 'pdga', 'Username', 'birthdate')));
...
function GetEventParticipants($eventId, $sortedBy, $search) {
    ...
    $query = "SELECT :User.id AS UserId, Username, Role, UserFirstName, UserLastName, UserEmail, :Player.firstname pFN, :Player.lastname pLN,
                               ...
                               UNIX_TIMESTAMP(SignupTimestamp) SignupTimestamp, :Classification.id AS ClassId, sfl_clubs.club_short AS Club_short
                  FROM :User
                  INNER JOIN :Player ON :Player.player_id = :User.Player
...
function SaveTextContent($page) {
    ...
    if (!$page->id) {
        $query = data_query("INSERT INTO :TextContent (Event, Title, Content, Date, Type, `Order`)
                          VALUES (%s, '%s', '%s', FROM_UNIXTIME(%d), '%s', %d)", esc_or_null($page->event, "int"), mysql_real_escape_string($page->title), mysql_real_escape_string($page->content), time(), mysql_real_escape_string($page->type), 0
        );
    } else {
        $query = data_query("UPDATE :TextContent
                              SET
                                 ...
                                 `Type` = '%s'
                                 WHERE id = %d", mysql_real_escape_string($page->title), mysql_real_escape_string($page->content), time(), $page->type, (int) $page->id
        );
    }
...
function InitializeAd($eventid, $contentId) {
    ...
    $query = data_query("INSERT INTO :AdBanner (Event, URL, ImageURL, LongData, ImageReference, Type, ContentId)
                       VALUES (%s, NULL, NULL, NULL, NULL, '%s', '%s')", esc_or_null($eventid, 'int'), ($eventid ? AD_EVENT_DEFAULT : AD_DEFAULT), mysql_real_escape_string($contentId));
...
function data_UpdateHoleResult($rrid, $playerid, $holeid, $result) {
    ...
    $query = data_query(
            "SELECT id FROM :HoleResult WHERE RoundResult = %d AND Player = %d   AND Hole = %d", $rrid, $playerid, $holeid
    );
    ...
    if (!mysql_num_rows($dbres)) {
        ...
        $query = data_query("INSERT INTO :HoleResult (Hole, RoundResult, Player, Result, DidNotShow, LastUpdated)
                          VALUES (%d, %d, %d, 0, 0, NOW())", $holeid, $rrid, $playerid);
        mysql_query($query);
        echo mysql_error();
    }
    ...
    $query = data_query("UPDATE :HoleResult SET Result = %d, DidNotShow = %d, LastUpdated = NOW() WHERE RoundResult = %d AND Hole = %d AND Player = %d", $result, $dns, $rrid, $holeid, $playerid);
...
function data_UpdateRoundResult($rrid, $modifyField = null, $modValue = null) {
    ...
    $query = data_query("UPDATE :RoundResult SET Result = %d, Penalty = %d, SuddenDeath = %d, Completed = %d,
                          ...
                          WHERE id = %d", $total, $penalty, $suddendeath, $complete, $dnf ? 1 : 0, $plusminus, $rrid);
    $res = mysql_query($query);
    if (!$res)
        return Error::Query($query);
...
function GetRoundResult($roundid, $playerid) {
    // TODO: potential race condition, should be fixed
    $query = data_query(
            "SELECT id FROM :RoundResult WHERE `Round` = %d AND Player = $playerid", $roundid, $playerid);
    $result = mysql_query($query);
    echo mysql_error();
...
function EditSection($roundId, $sectionId, $priority, $startTime) {
    ...
    $query = data_query("UPDATE :ClassOnRound SET Priority = %s, StartTime = FROM_UNIXTIME(%s) WHERE Round = %d AND Classification = %d", $priority, $startTime, $roundId, $classId);
...
function MarkEventFeePayment($eventid, $participationId, $payment) {
    ...
    $query = data_query("UPDATE :Participation SET EventFeePaid = FROM_UNIXTIME(%s), Approved = 1 WHERE id = %d AND Event = %d", ($payment ? time() : "NULL"), (int) $participationId, (int) $eventid);
...
function SetRoundDetails($roundid, $date, $startType, $interval, $valid, $course) {
    ...
    $query = data_query("UPDATE :Round SET StartTime = FROM_UNIXTIME(%d), StartType = '%s', `Interval` = %d, ValidResults = %d, Course = %s WHERE id = %d", (int) $date, mysql_real_escape_string($startType), (int) $interval, $valid ? 1 : 0, esc_or_null($course, 'int'), $roundid);
...
function SaveHole($hole) {
    ...
    if ($hole->id) {
        ...
        $query = data_query("UPDATE :Hole SET Par = %d, Length = %d, HoleNumber = %d WHERE id = %d", (int) $hole->par, (int) $hole->length, $hole->holeNumber, (int) $hole->id);
    } else {
        $query = data_query("INSERT INTO :Hole (Par, Length, Course, HoleNumber) VALUES (%d, %d, %d, %d)", (int) $hole->par, (int) $hole->length, (int) $hole->course, $hole->holeNumber);
    }
...
function RemovePlayersFromRound($roundid, $playerids = null) {
    ...
    $q1 = data_query("SELECT :SectionMembership.id FROM :SectionMembership
      ...
      WHERE :Section.Round = %d AND :Participation.Player IN (%s)", $roundid, implode(", ", $playerids));
    ...
    $query = data_query("DELETE FROM :SectionMembership WHERE id IN (%s)", implode(", ", $ids));
...
function AdjustSection($sectionid, $priority, $sectionTime, $present) {
    ...
    $query = data_query("UPDATE :Section SET Priority = %d, StartTime = FROM_UNIXTIME(%s), Present = %d WHERE id = %d", $priority, esc_or_null($sectionTime, 'int'), $present ? 1 : 0, $sectionid
    );
...
function InsertGroup($group) {
    ...
    foreach ($group['People'] as $player) {
        $query = data_query("INSERT INTO :StartingOrder
                     ...
                     VALUES (%d, FROM_UNIXTIME(%d), %s, %d, %d)", $player['PlayerId'], $group['StartingTime'], esc_or_null($group['StartingHole'], 'int'), $group['PoolNumber'], $group['Section']);
        mysql_query($query);
        echo mysql_error();
    }
}
...
function InsertGroupMember($data) {
    $query = data_query("INSERT INTO :StartingOrder
                    (Player, StartingTime, StartingHole, PoolNumber, Section)
                    VALUES (%d, FROM_UNIXTIME(%d), %s, %d, %d)", $data['Player'], $data['StartingTime'], esc_or_null($data['StartingHole'], 'int'), $data['PoolNumber'], $data['Section']);
    mysql_query($query);
    echo mysql_error();
}
...
function SaveParticipationResults($entry) {
    $query = data_query("UPDATE :Participation
                           ...
                           DidNotFinish = %d,
                           TournamentPoints = %d
                        WHERE id = %d", $entry['OverallResult'], $entry['Standing'], $entry['DidNotFinish'], $entry['TournamentPoints'], $entry['id']
    );
...
function CreateFileRecord($filename, $displayname, $type) {
    $query = data_query("INSERT INTO :File (Filename, DisplayName, Type) VALUES
                     ('%s', '%s', '%s')", mysql_real_escape_string($filename), mysql_real_escape_string($displayname), mysql_real_escape_string($type)
    );
...
function SaveAd($ad) {
    $query = data_query("UPDATE :AdBanner SET URL = %s, ImageURL = %s, LongData = %s, ImageReference = %s, Type = %s WHERE id = %d", esc_or_null($ad->url), esc_or_null($ad->imageURL), esc_or_null($ad->longData), esc_or_null($ad->imageReference), esc_or_null($ad->type), $ad->id
    );
    $res = mysql_query($query);
    if (!$res)
        return Error::Query($query);
...
function SetParticipantClass($eventid, $playerid, $newClass) {
    $query = data_query("UPDATE :Participation SET Classification = %d WHERE Player = %d AND Event = %d", $newClass, $playerid, $eventid);
    $res = mysql_query($query);
    if (!$res)
        return Error::Query($query);
...
function SaveCourse($course) {
    ...
    if ($course['id']) {
        ...
        $query = data_query("UPDATE :Course
                          ...
                          Link = '%s',
                          Map = '%s'
                          WHERE id = %d", mysql_real_escape_string($course['Name']), mysql_real_escape_string($course['Description']), mysql_real_escape_string($course['Link']), mysql_real_escape_string($course['Map']), $course['id']
        );
        $res = mysql_query($query);
        if (!$res)
            return Error::Query($query);
        ...
        $query = data_query("INSERT INTO :Course (Name, Description, Link, Map, Event)
                          VALUES ('%s', '%s', '%s', '%s', %s)", mysql_real_escape_string($course['Name']), mysql_real_escape_string($course['Description']), mysql_real_escape_string($course['Link']), mysql_real_escape_string($course['Map']), esc_or_null($eventid, 'int'));
        $res = mysql_query($query);
        if (!$res)
            return Error::Query($query);
        return mysql_insert_id();
...
function SaveTournamentStanding($item) {
    ...
    $query = data_query("UPDATE :TournamentStanding
                        ...
                        WHERE Player = %d AND Tournament = %d", $item['OverallScore'], $item['Standing'], $item['PID'], $item['TID']
    );
...
function PlayerParticipating($eventid, $userid) {
    $query = data_query("SELECT :Participation.id FROM :Participation
                        INNER JOIN :Player ON :Participation.Player = :Player.player_id
                        INNER JOIN :User ON :User.Player = :Player.player_id
                        WHERE :User.id = %d AND :Participation.Event = %d", $userid, $eventid);
    $res = mysql_query($query);
...
function GetUserFees($playerid, $year) {
    /*
    $lquery = data_query("SELECT 1 FROM :LicensePayment WHERE Player = %d AND Year = %d", $playerid, $year);
    $result1 = mysql_query($lquery);
    $license = mysql_num_rows($result1);
    mysql_free_result($result1);

    $mquery = data_query("SELECT 1 FROM :MembershipPayment WHERE Player = %d AND Year = %d", $playerid, $year);
...
function SetTournamentTieBreaker($tournament, $player, $value) {
    ...
    $query = data_query("UPDATE :TournamentStanding SET TieBreaker = %d WHERE Player = %d AND Tournament = %d", $value, $player, $tournament);
    mysql_query($query);
    echo mysql_error();
}
...
//arttu 28.3.2012
// gets array containing clubs that are registered in database
function GetClubs($searchQuery = '') {
    ...
    $retValue = array();
    //data_query();
    //club_id 	club_name 	club_short 	sfl_club 	visma_club
    $query = data_query("SELECT club_id AS Id, club_name AS Name, club_short AS Short_name FROM sfl_clubs");
    $result = mysql_query($query);
    if (!$result)
        return Error::Query($query);
    if (mysql_num_rows($result) > 0) {
...
function SetPlayerToWaitinglist($playerid, $eventid, $classid) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    $query = data_query("SELECT COUNT(Player) AS PlayersWaiting FROM :Waitinglist WHERE Event = %d AND Classification = %d", $eventid, $classid);
    $result = mysql_query($query);
    if (mysql_num_rows($result) == 1) {
        $row = mysql_fetch_assoc($result);
        $signUpNumber = $row['PlayersWaiting'];
    }

    mysql_free_result($result);
    $signUpNumber++;

    $retValue = $signUpNumber;

    $query = data_query("INSERT INTO :Waitinglist (Player, Event, Classification, Signup_number) VALUES (%d, %d, %d, %d);", (int) $playerid, (int) $eventid, (int) $classid, (int) $signUpNumber);
    ...
    if (!mysql_query($query)) {
        ...
        $err->title = "error_db_query";
        $err->description = translate("error_db_query_description");
        $err->internalDescription = "Failed SQL INSERT query (Waitinglist)";
        $err->function = "SetPlayerToWaitinglist()";
        $err->IsMajor = true;
        $err->data = "Player id: " . $playerid .
                "; Event id: " . $eventid .
                "; Classification id: " . $classid;
...
function checkIfPlayersInWaitingList($eventId, $classId = null) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
...
function checkIfUserSignuppable($playerId, $eventId) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    ...
    $query = data_query("SELECT Player FROM :Waitinglist WHERE Event = %d AND Player = %d", $eventId, $playerId);
    $result = mysql_query($query);
    if (mysql_num_rows($result) > 0) {
        return false;
    }
...
function checkEventViewVisibility($eventId, $viewName) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
...
function PublishPage($eventId, $view) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    ...
    if ($rowFound) {
        ...
        $query = data_query("UPDATE :Event_views_visibility SET visibility = '%s' WHERE event = %d AND view = '%s'", "public", $eventId, $view);
        if (!mysql_query($query))
            return false;
    }else {
        $query = data_query("INSERT INTO :Event_views_visibility (event, view, visibility) VALUES (%d, '%s' ,'%s')", $eventId, $view, "public");
        logToFile($query, "kysse");
        if (!mysql_query($query))
            return false;
    }
    return true;
...
function UnpublishPage($eventId, $view) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    ...
    if ($rowFound) {
        ...
        $query = data_query("UPDATE :Event_views_visibility SET visibility = '%s' WHERE event = %d AND view = '%s'", "private", $eventId, $view);
        if (!mysql_query($query))
            return false;
    }else {
        $query = data_query("INSERT INTO :Event_views_visibility (event, view, visibility) VALUES (%d, '%s' ,'%s')", $eventId, $view, "private");
...
function CheckEventViewsPublicity($eventId, $view) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
...
function AddEventViewsVisibilities($eventId, $publicity) {
    $eventViews = array('cancelsignup', 'competitors', 'course', 'custom', 'index', 'leaderboard', 'newsarchive', 'newsitem', 'payment', 'results', 'schedule', 'signupinfo', 'waitinglist');

    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    foreach ($eventViews as $view) {
        $query = data_query("INSERT INTO :Event_views_visibility (event, view, visibility) VALUES (%d, '%s' ,'%s')", $eventId, $view, $publicity);
        mysql_query($query);
    }
}

function GetEventClassQueuers($eventId, $classId) {
    ...
    $sortOrder = " Signup_number";

    $prequery = "SELECT :User.id AS UserId, Username, Role, UserFirstName, UserLastName, UserEmail, :Player.firstname pFN, :Player.lastname pLN,
                                :Player.email pEM,
                               :Player.player_id AS PlayerId, pdga PDGANumber, Sex, YEAR(birthdate) YearOfBirth, :Classification.Name AS ClassName,
                               :Waitinglist.id AS WaitinglistID, :Waitinglist.Signup_number,
                               UNIX_TIMESTAMP(Signup_Timestamp) Timestamp, :Classification.id AS ClassId, sfl_clubs.club_short AS Club_short
    ...
    $query = data_query($prequery, $eventId, $classId);
    $result = mysql_query($query);
    require_once('core/player.php');
    //echo mysql_error();
    if (mysql_num_rows($result) > 0) {
...
function RemovePlayerFromQueue($playerId, $eventId, $classId) {

    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    $query = data_query("DELETE FROM :Waitinglist WHERE Event = %d AND Player = %d AND Classification = %d", $eventId, $playerId, $classId);
    $result = mysql_query($query);
    return $result;
}

function UpdateSignupNumbers($eventId, $classId) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }

    $prequery = "SELECT :Player.player_id AS PlayerId, :Waitinglist.id AS WaitinglistID, :Waitinglist.Signup_number,
                               UNIX_TIMESTAMP(Signup_Timestamp) Timestamp, :Classification.id AS ClassId
                  FROM :User
                  INNER JOIN :Player ON :Player.player_id = :User.Player
                  INNER JOIN :Waitinglist ON :Waitinglist.Player = :Player.player_id AND :Waitinglist.Event = " . $eventId . "
                  INNER JOIN :Classification ON :Waitinglist.Classification = :Classification.id
                  WHERE :Waitinglist.Event = %d AND :Waitinglist.Classification = %d
                  ORDER BY :Waitinglist.Signup_number 
                  ";

    $query = data_query($prequery, $eventId, $classId);
    $playersOnQueue = array();
    $result = mysql_query($query);
    if (mysql_num_rows($result) > 0) {
        while ($row = mysql_fetch_assoc($result)) {
            ...
            $pdata['playerId'] = $row['PlayerId'];
            $pdata['signupNumber'] = $row['Signup_number'];
            //logListToFile($row);
            $pdata['signupTimestamp'] = $row['Timestamp'];
            $pdata['classId'] = $row['ClassId'];
            $playersOnQueue[] = $pdata;
        }
        if (isset($playersOnQueue)) {
            $i = 1;
            foreach ($playersOnQueue as $player) {

               $updateQuery = 'UPDATE :Waitinglist SET Signup_number = %d WHERE :Waitinglist.Player = %d AND :Waitinglist.Event = %d AND :Waitinglist.Classification = %d';
                $updateQuery = data_query($updateQuery, $i, $player['playerId'], $eventId, $classId);
...
function fetchUserInfoFromSfl($pdga) {
    $dbError = InitializeDatabaseConnection();
    if ($dbError) {
        return $dbError;
    }
    $query = "SELECT `firstname`, `lastname`, `birthdate`, `sex`, `email`,`club_id`,`forum_name`, `pdga` FROM `sfl_player` WHERE `pdga` = $pdga"; // data_query("DELETE FROM :Waitinglist WHERE Event = %d AND Player = %d AND Classification = %d", $eventId, $playerId, $classId);
    $result = mysql_query($query);
    $pdata = array();
    if (mysql_num_rows($result) == 1) {
        $row = mysql_fetch_assoc($result);
        $pdata = array(array('field' => 'firstname',
                'value' => $row['firstname']),
            ...
            array('field' => 'club_id',
                'value' => $row['club_id']),
            array('field' => 'forum_name',
                'value' => $row['forum_name']),
            array('field' => 'pdga',
                'value' => $row['pdga']));

        /* $pdata['firstname'] = $row['firstname'];
          $pdata['lastname'] = $row['lastname'];
          $pdata['birthdate'] = $row['birthdate'];
          $pdata['sex'] = $row['sex'];
          $pdata['email'] = $row['email'];
          $pdata['club_id'] = $row['club_id'];
          $pdata['forum_name'] = $row['forum_name'];
          $pdata['pdga'] = $row['pdga'];
         * 
         */
    } else {
